...
def simple_demo():
    ...
    while True:
        if not c.cv_image is None:
            c.binary_image = cv2.inRange(c.cv_image, np.float32(c.blue_lower_bound,c.green_lower_bound,c.red_lower_bound), np.float32(c.blue_upper_bound,c.green_upper_bound,c.red_upper_bound))
